...
// ── Catálogo geral: todas as categorias vivem em index.html ──────────────
// O índice de busca foi herdado dos catálogos por categoria, onde cada linha
// tinha sua própria página. Aqui os mesmos destinos viram âncoras locais.
function parketResolveUrl(url) {
  if (!url) return url;
  if (url.startsWith('index.html#')) {
    const anchor = url.slice('index.html'.length);
    // A obra do Fasano é apresentada dentro da coleção Brazil de decks.
    return anchor === '#colecao-decks-fasano-trancoso' ? '#colecao-decks-brazil' : anchor;
  }
  let m;
  if ((m = url.match(/^(pisos|decks)\.html#(.+)$/))) return `#colecao-${m[1]}-${m[2]}`;
  if ((m = url.match(/^(pisos|decks)-(.+)\.html$/))) return `#colecao-${m[1]}-${m[2]}`;
  if ((m = url.match(/^(pisos|decks|forros|paineis|escadas|portas|marcenarias)\.html$/))) return `#produto-${m[1]}`;
  if (url === 'fachadas.html') return '#produto-paineis';
  if (url.startsWith('texturas')) return '#texturas';
  if (url === 'catalogo.html') return '#products-index';
  return url;
}
...
const PARKET_PAGES = [
  ...
  { cat: 'Marcenarias', name: 'Marcenarias', url: 'marcenarias.html', tags: 'marcenaria marcenarias armario armário armários painel cabeceira estante closet sob medida' },
  { cat: 'Marcenarias', name: 'Armários sob medida', url: 'marcenarias.html', tags: 'marcenaria armario armário closet sob medida' },
  { cat: 'Marcenarias', name: 'Cabeceiras', url: 'marcenarias.html', tags: 'marcenaria cabeceira quarto cama' },
  { cat: 'Marcenarias', name: 'Estantes e home', url: 'marcenarias.html', tags: 'marcenaria estante home theater prateleira living' },
  { cat: 'Marcenarias', name: 'Muxarabi', url: 'marcenarias.html', tags: 'marcenaria muxarabi muxarabie divisoria divisória ripado' },
...
class ParketNav extends HTMLElement {
  ...
  _renderResults(query) {
    ...
    this._results.innerHTML = list.map(p => `
      <a class="parket-search-result" href="${parketResolveUrl(p.url)}">
